...
removeObscenityWord = on_command(
    "违禁词删除",
    priority=10,
    block=True,
    permission=GROUP_ADMIN | GROUP_OWNER | SUPERUSER,
)
addObscenityWord = on_command(
    "违禁词添加",
    priority=10,
    block=True,
    permission=GROUP_ADMIN | GROUP_OWNER | SUPERUSER,
)
rule = Rule(is_bot_op, isnot_op)
f_word = on_message(rule=rule, priority=20, block=False)
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if re.search(obs_re, event.message.extract_plain_text()):
        try:
            ...
            await f_word.finish(
                f'检测到敏感词，已撤回，已禁言"{event.sender.nickname}" 60s'
            )
